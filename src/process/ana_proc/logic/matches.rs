...
use crate::process::ana_proc::logic::anakind::{AnalysisKind, SimulationActionCriterion, SimulationLoopCriterion};
...
impl AnalysisProcessManager {
    ...
    pub fn add_simulation_matches_in_analysis(&self,
        ...
        for frt_elt in global_frontier(&interaction,&None) {
            let canal_ids_of_targets = self.co_localizations.get_coloc_ids_from_lf_ids(&frt_elt.target_lf_ids);
            // ***
            ...
            for (canal_id, canal_flag) in flags.canals.iter().enumerate() {
                let canal_trace = self.multi_trace.get(canal_id).unwrap();
            ...
            for tract in act_left_to_match {
                ...
                let tract_coloc_id = self.co_localizations.get_lf_coloc_id(tract.lf_id).unwrap();
                if ok_canals.contains(&tract_coloc_id) {
                    ...
                    let canal_trace = self.multi_trace.get(tract_coloc_id).unwrap();
                    // *
            ...
            if ok_to_simulate {
                {
                    *next_child_id = *next_child_id +1;
                    ...
                    let generic_step = GenericStep{parent_id,
                        ...
                        kind:AnalysisStepKind::Execute(frt_elt.clone(),
                                                       consu_set,
                                                       to_simulate.clone())};
    ...
    pub fn add_action_matches_in_analysis(&self,
        ...
        for (canal_id,canal_flags) in flags.canals.iter().enumerate() {
            let trace = self.multi_trace.get(canal_id).unwrap();
