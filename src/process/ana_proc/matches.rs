...
pub fn add_action_matches_in_analysis(gen_ctx : &GeneralContext,
                                      parent_id    : u32,
    ...
    for frt_elt in global_frontier(&interaction,&Some(head_actions)) {
        *id_as_child = *id_as_child +1;
        ...
        for lf_id in &frt_elt.target_lf_ids {
            canal_ids_of_targets.insert( gen_ctx.get_lf_coloc_id(*lf_id).unwrap() );
        }
...
pub fn add_simulation_matches_in_analysis(gen_ctx : &GeneralContext,
                                          parent_id    : u32,
                                          interaction : &Interaction,
    ...
    for frt_elt in global_frontier(&interaction,&None) {
        ...
        if multi_trace.length() > 0 {
            ...
            for tract in act_left_to_match {
                ...
                let tract_coloc_id = gen_ctx.get_lf_coloc_id(tract.lf_id).unwrap();
                if ok_canals.contains(&tract_coloc_id) {
                    ...
                    let mut gotit = false;
                    for (canal_id, canal) in multi_trace.canals.iter().enumerate() {
                        let canal_lifelines = gen_ctx.co_localizations.get(canal_id).unwrap();
