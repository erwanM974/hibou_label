        ...
        Ok( gen_ctx ) => {
            ...
            match parse_htf_file(&gen_ctx, htf_file_path) {
                ...
                Ok( (co_localizations,multi_trace) ) => {
                    ...
                    // ***
                    let only_at_end : bool;
                    if matches.is_present("only_at_end") {
                        only_at_end = true;
                    } else {
                        only_at_end = false;
                    }
                    // ***
                    let one_per_compo_max : bool;
                    if matches.is_present("one_per_compo_max") {
                        one_per_compo_max = true;
                    } else {
                        one_per_compo_max = false;
                    }
                    // ***
                    ...
                    if matches.is_present("name") {
                        ...
                        mutant_name = format!("{}_insert_noise_mutant",mu_name);
                    }
                    // ***
                    let mutant_file_path = generate_insert_noise_mutant(&gen_ctx,&co_localizations,&multi_trace,parent_folder,&mutant_name,max_num_inserts,one_per_compo_max,only_at_end);
                    // ***
