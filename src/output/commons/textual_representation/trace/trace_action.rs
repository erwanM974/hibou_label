...
use crate::output::commons::textual_convention::{SYNTAX_EMISSION, SYNTAX_RECEPTION};

pub fn trace_actions_as_text(gen_ctx : &GeneralContext, actions : &HashSet<TraceAction>) -> String {
    if actions.len() == 1 {
        return trace_action_as_text(gen_ctx,actions.iter().next().unwrap());
    } else {
        let mut actions_as_text= vec![];
        for action in actions.iter().sorted() {
            actions_as_text.push( trace_action_as_text(gen_ctx,action) );
        }
...
fn trace_action_as_text(gen_ctx : &GeneralContext, action : &TraceAction) -> String {
